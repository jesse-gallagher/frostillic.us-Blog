...
public class UserInfo implements Serializable {
	...
	public boolean isAdmin() {
		return FrameworkUtils.getDatabase().queryAccessRoles(getName()).contains("[Admin]"); //$NON-NLS-1$
	}
	public boolean isDebug() {
		return FrameworkUtils.getDatabase().queryAccessRoles(getName()).contains("[Debug]"); //$NON-NLS-1$
	}
	...
	public String getDisplayName() {
		...
		List<?> names = FrameworkUtils.getSession().evaluate(" @NameLookup([NoUpdate]; '" + user.getName() + "'; 'FirstName')[1]:@NameLookup([NoUpdate]; '" + user.getName() + "'; 'LastName')[1] "); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		String firstName = names.size() < 1 ? "" : names.get(0).toString(); //$NON-NLS-1$
		String lastName = names.size() < 2 ? "" : names.get(1).toString(); //$NON-NLS-1$
		if(StringUtil.isNotEmpty(firstName)) {
			if(StringUtil.isNotEmpty(lastName)) {
				if(lastName.length() > 1) {
					return firstName + " " + lastName.charAt(0) + "."; //$NON-NLS-1$ //$NON-NLS-2$
				}
	...
	public String getProfileImageUrl() {
		...
		return "https://secure.gravatar.com/avatar/" + MD5Util.md5Hex(email); //$NON-NLS-1$
	}
		...
		public DataObject getValue(final Object key) {
			return new DataObject() {
				...
				public String getValue(final Object key2) {
					return "https://secure.gravatar.com/avatar/" + MD5Util.md5Hex(String.valueOf(key)) + "?s=" + key2; //$NON-NLS-1$ //$NON-NLS-2$
				}
