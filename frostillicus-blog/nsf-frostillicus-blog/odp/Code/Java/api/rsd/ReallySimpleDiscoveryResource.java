...
public class ReallySimpleDiscoveryResource {
	...
	public Document get() throws XMLException {
		// TODO move to JAX-B
		var doc = DOMUtil.createDocument();

		var rsd = DOMUtil.createElement(doc, "rsd"); //$NON-NLS-1$
		doc.appendChild(rsd);
		...
		var service = DOMUtil.createElement(doc, rsd, "service"); //$NON-NLS-1$
		DOMUtil.createElement(doc, service, "engineName").setTextContent(translation.getString("appTitle")); //$NON-NLS-1$ //$NON-NLS-2$
		DOMUtil.createElement(doc, service, "engineLink").setTextContent(translation.getString("baseUrl")); //$NON-NLS-1$ //$NON-NLS-2$
		DOMUtil.createElement(doc, service, "homePageLink").setTextContent(uriInfo.getBaseUri().toString()); //$NON-NLS-1$

		var apis = DOMUtil.createElement(doc, service, "apis"); //$NON-NLS-1$
		serviceClasses.forEach(clazz -> {
			var def = clazz.getAnnotation(RSDService.class);

			var api = DOMUtil.createElement(doc, apis, "api"); //$NON-NLS-1$
